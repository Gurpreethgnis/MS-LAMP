...
use vars qw($ll $ul $upperm $lowerm @uppermz @lowermz $mass $fa $gp $gl $pk $pr $round $st $cq $sfl $mac $mak $mono $di $diac $diak $diacak $diakac $tri $triac $all $spl $opl $flor $clor $brom $iod $sl $sp $workbook);
...
use vars qw($massentered $range @mzval $lenmz $counterentirelipidome @m2mono %f2l);
use vars qw($open $relativeintensityvalue @lstrcq $ogll @lstrogll $lstrcq @lstrfa @lstrgl @lstrgp @lstrpk @lstrpr @lstrst @lstrsl @lstrsp);
...
use vars qw($hydromono $hydroavg $amnmono $amnavg $sodmono $sodavg $clmono $clavg $litmono $litavg $potmono $potavg);
#**************************** Main Window Display********************************
$m = new MainWindow;
$m->geometry("1010x620+0+0");
...
$counterentirelipidome=0;$counterfasearch=0;$countergpsearch=0;$counterglsearch=0;$counterstsearch=0;
	...
	$counterkpos=0;$counter2khpos=0;$counterk2hneg=0;$ll=$ul=0;


	$m->title("MS - LAMP - M. tb Lipidome MS-LAMP - Query Type 3 : Molecular Mass or m/z range ");
	$sfmn=$m->Frame(-background=>$col2)->pack(-side=>top,-fill=>x);
	$sfmm=$sfmn->Frame(-background=>$col4,-relief => ridge,-borderwidth=>3)->pack(-side=>top,-fill=>x);
	$sfmzq=$sfmm->Label( -text=>"",-font=>"{$fontty} 5 bold",-background =>$col1,-foreground =>$col5)->pack(-side=>top,-fill=>x);
	$sfmzr=$sfmm->Label( -text=>"Query Type 3 : Molecular Mass or m/z range",-font=>"{$fontty} 18 bold",-background =>$col1,-foreground =>$col5,-borderwidth=>3)
			->pack(-side=>top,-fill=>x);
	$sfmzq=$sfmm->Label( -text=>"",-font=>"{$fontty} 5 bold",-background =>$col1,-foreground =>$col5)->pack(-side=>top,-fill=>x);
	
	$space=$sfmn->Frame(-background=>"$col5")->pack(-side=>left,-anchor=>center);
	$spacel=$space->Label(-text=>"\t\t\t\t",-background=>$col2)->pack(-side=>left);
	$sfm=$sfmn->Frame(-background=>$col5)->pack(-side=>left,-fill=>x);
	$blank=$sfm->Label(-text=>"        ",-background=>$col2,-font=>"{$fontty} 10")->pack(-side=>top,-fill=>x);
	$framefileformat=$sfmn->Frame(-background=>$col2)->pack(-side=>left,-anchor=>center);
	$labelofscroll=$sfm->Label(-text=> "Lower limit",-font=>"{$fontty} 12 bold",-background=>$col2,-foreground=>$col4)->pack(-side=>left,-anchor=>center);	
		$lowerm=$sfm-> Entry(-relief=>sunken,-borderwidth=>5)->pack(-side=>left,-anchor=>center);
		
	$labelofscroll=$sfm->Label(-text=> "Upper limit",-font=>"{$fontty} 12 bold",-background=>$col2,-foreground=>$col4)->pack(-side=>left,-anchor=>center);	
		$upperm=$sfm-> Entry(-relief=>sunken,-borderwidth=>5)->pack(-side=>left,-anchor=>center);
	$masschoice=0;
	$monoiso=$sfm->Radiobutton(-text=>"Monoisotopic ",-font=>"{times new roman} 10 bold",-value=>"0",-variable=>\$masschoice,-foreground=>$col4,-background=>$col2)->pack(-side=>left,-anchor=>center);
	$averagemass=$sfm->Radiobutton(-text=>"Average ",-font=>"{times new roman} 10 bold",-value=>"1",-variable=>\$masschoice,-foreground=>$col4,-background=>$col2)->pack(-side=>left,-anchor=>center);

		
	$blank2=$m->Label(-text=>"",-background=>$col2,-font=>"{$fontty} 10")->pack(-side=>top,-fill=>x);
	$iontype=$m->Label(-text=>"Choose Molecular Mass / Type of Ion",-font=>"{$fontty} 12 bold",-background=>$col1,-foreground=>$col4)->pack(-side=>top,-anchor=>center,-fill=>x);
	$blank3=$m->Label(-text=>"",-background=>white,-font=>"{$fontty} 4")->pack(-side=>top,-fill=>x);
	$frameiontype=$m->Frame(-background=>white,-foreground=>black)->pack(-side=>top,-anchor=>center);
		$blank4=$m->Label(-text=>"",-background=>white,-font=>"{$fontty} 4")->pack(-side=>top,-fill=>x);
$fontsizz=8.5;
	...
	$searchcriterion=$m->Label(-text=>"Which Lipid(s) ?",-font=>"{fontty} 12 bold",-foreground=>$col4,-background=>$col1)->pack(-side=>top,-anchor=>center,-fill=>x);
		$blank5=$m->Label(-text=>"",-background=>white,-font=>"{times new roman} 4")->pack(-side=>top,-fill=>x);
...
$spaceaphospho=$m->Label(-text=>"___________________________________________________________________________________________________________________________________________________",-background=>white,-foreground=>$col4)->pack(-side=>top);
	...
	$customhelp=$framecustom->Button(-text=>"Need Help ?",-font=>"{$fontty} 10 bold",-background=>$col2,-foreground=>$col3,-command=>\&s10)->pack(-side=>left);
	$spaceacustom=$m->Label(-text=>"___________________________________________________________________________________________________________________________________________________",-background=>white,-foreground=>$col4)->pack(-side=>top);
...
{	$sfmn->packForget();
	...
	$blank4->packForget();
	$blank5->packForget();
	$framefileformat->packForget();
	$iontype->packForget();
	$labelofscroll->packForget();	
...
sub countertallygl{$counterglsearch=0;&searchallact;$glsearch->configure(-command=>\&glsearchdis);}
...
sub opening
{
	...
	$ul=$ul+1;
	@mzval=();
for($numcount=$ll;$numcount<$ul;$numcount++)
{$mzval[$round] =$numcount;
$round++;
}	
}
...
$lvck=$lowerm->get();
$uvck=$upperm->get();
if ($lvck eq "") { $lvck="Value not entered";} 
if ($uvck eq "") { $uvck="Value not entered";} 
if ($countermhpos eq 1){$qurdion='[M+H]+'}		
	elsif($countermampos eq 1){$qurdion='Ion type : [M+NH4]+'}		
	elsif($counternapos eq 1){$qurdion='Ion type : [M+Na]+'}
	elsif($counternahpos eq 1){$qurdion='Ion type : [M+2Na-H]+'}
	elsif($countermhneg eq 1){$qurdion='Ion type : [M-H]-'}
	elsif($counterachneg eq 1){$qurdion='Ion type : [M+HOAc-H]-'}
	elsif($counternahneg eq 1){$qurdion='Ion type : [M+Na-2H]-'}
	elsif($counterclneg eq 1){$qurdion='Ion type : [M+Cl]-'}
	elsif($counterlipos eq 1){$qurdion='Ion type : [M+Li]+'}
	elsif($counter2lihpos eq 1){$qurdion='Ion type : [M+2Li-H]+'}
	elsif($counterli2hneg eq 1){$qurdion='Ion type : [M+Li-2H]-'}
	elsif($counterkpos eq 1){$qurdion='Ion type : [M+K]+'}
	elsif($counter2khpos eq 1){$qurdion='Ion type : [M+2K-H]+'}
	elsif($counterk2hneg eq 1){$qurdion='Ion type : [M+K-2H]-'}
	elsif($countermmass eq 1){$qurdion='Ion type : Molecular Mass'}
	else{$qurdion='No Molecular Mass/ Type of Ion selected'}

if ((($lvck eq $uvck)|($lvck>$uvck))&&(($counterentirelipidome eq 1) | ($countermainhalo eq 1)| ($countercustomcheck eq 1) | ($countersulfo eq 1) | ($countermonoglycero eq 1) | ($counterdiglycero eq 1)|($countertriglycero eq 1)|($counterotherglycero eq 1)|($countermainphospho eq 1) | ($countersphingophospho eq 1)|($counterotherphospho eq 1)| ($countermainphospho eq 1) | ($countersphingophospho eq 1) | ($countergpsearch eq 1) | ($counterotherphospho eq 1) | ($counterfasearch eq 1) | ($countergpsearch eq 1) | ($counterglsearch eq 1) | ($counterpksearch eq 1) | ($counterprsearch eq 1) | ($counterspsearch eq 1)| ($counterslsearch eq 1) | ($counterstsearch eq 1)|($countermainglycero eq 1))&&((($counter2lihpos eq 1)|($counterlipos eq 1)|($counterli2hneg eq 1) | ($countermhpos eq 1) | ($countermampos eq 1)|($counternapos eq 1) | ($counternahpos eq 1) | ($countermhneg eq 1)|($counterachneg eq 1) | ($counternahneg eq 1)|  ($countermmass eq 1) | ($counterclneg eq 1) |($counterkpos eq 1)| ($counter2khpos eq 1) |($counterk2hneg eq 1))))
{
$errortoptitle="Result of the query";
$errortitle="Oops ! There are no results matching to the query";
$errormess="\nLower and Upper limit have \n not been entered properly.\n";  
$errorbottom="Back";
...
$label=$nodata->Label(-text=>"\n$errormess\n",-font=>"{times new roman} 36 ",-background=>$col,-foreground=>$col3)->pack(-side=>'bottom',-fill=>x);


}

if (($lvck<$uvck)&&($round eq 0)&&(($counterentirelipidome eq 1) | ($countermainhalo eq 1)| ($countercustomcheck eq 1) | ($countersulfo eq 1) | ($countermonoglycero eq 1) | ($counterdiglycero eq 1)|($countertriglycero eq 1)|($counterotherglycero eq 1)|($countermainphospho eq 1) | ($countersphingophospho eq 1)|($counterotherphospho eq 1)| ($countermainphospho eq 1) | ($countersphingophospho eq 1) | ($countergpsearch eq 1) | ($counterotherphospho eq 1) | ($counterfasearch eq 1) | ($countergpsearch eq 1) | ($counterglsearch eq 1) | ($counterpksearch eq 1) | ($counterprsearch eq 1) | ($counterspsearch eq 1)| ($counterslsearch eq 1) | ($countermainglycero eq 1)|($counterstsearch eq 1))&&((($counter2lihpos eq 1)|($counterlipos eq 1)|($counterli2hneg eq 1) | ($countermhpos eq 1) | ($countermampos eq 1)|($counternapos eq 1) | ($counternahpos eq 1) | ($countermhneg eq 1)|($counterachneg eq 1) | ($counternahneg eq 1)|  ($countermmass eq 1) | ($counterclneg eq 1) |($counterkpos eq 1)| ($counter2khpos eq 1) |($counterk2hneg eq 1))))
{
$errortoptitle="Result of the query";
$errortitle="Oops ! There are no results matching to the query ";
...
if ((($lvck eq 0)|($lvck < 0))&&(($uvck eq 0)|($uvck < 0))&&($round eq 0)&&($counterentirelipidome eq 0) && ($countermainhalo eq 0)&&  ($countermainglycero eq 0)&&($countercustomcheck eq 0) && ($countersulfo eq 0) && ($countermonoglycero eq 0)&&($counterdiglycero eq 0)&&($countertriglycero eq 0)&&($counterotherglycero eq 0)&&($countermainphospho eq 0) && ($countersphingophospho eq 0)&&($counterotherphospho eq 0)&& ($countermainphospho eq 0) && ($countersphingophospho eq 0) && ($countergpsearch eq 0) && ($counterotherphospho eq 0) && ($counterfasearch eq 0) && ($countergpsearch eq 0) && ($counterglsearch eq 0) && ($counterpksearch eq 0) && ($counterprsearch eq 0) && ($counterspsearch eq 0)&& ($counterslsearch eq 0) && ($counterstsearch eq 0)&&(($counter2lihpos eq 1)|($counterlipos eq 1)|($counterli2hneg eq 1) | ($countermhpos eq 1) | ($countermampos eq 1)|($counternapos eq 1) | ($counternahpos eq 1) | ($countermhneg eq 1)|($counterachneg eq 1) | ($counternahneg eq 1)|  ($countermmass eq 1) | ($counterclneg eq 1) |($counterkpos eq 1)| ($counter2khpos eq 1) |($counterk2hneg eq 1)))
{
$errortoptitle="Result of the query";
$errortitle="Oops ! There are no results matching to the query ";
...
if (($round > 0)&&($counter2lihpos eq 0)&&($counterlipos eq 0)&&($counterli2hneg eq 0) && ($countermhpos eq 0)&& ($countermampos eq 0)&&($counternapos eq 0) && ($counternahpos eq 0)&& ($countermhneg eq 0)&&($counterachneg eq 0) && ($counternahneg eq 0)&&  ($countermmass eq 0) && ($counterclneg eq 0) &&($counterkpos eq 0)&& ($counter2khpos eq 0) &&($counterk2hneg eq 0)&&(($counterentirelipidome eq 1) | ($countermainhalo eq 1)| ($countercustomcheck eq 1) | ($countersulfo eq 1) | ($countermonoglycero eq 1) | ($counterdiglycero eq 1)|($countertriglycero eq 1)|($counterotherglycero eq 1)|($countermainphospho eq 1) | ($countersphingophospho eq 1)|($counterotherphospho eq 1)| ($countermainphospho eq 1) | ($countersphingophospho eq 1) | ($countergpsearch eq 1) | ($counterotherphospho eq 1) | ($counterfasearch eq 1) | ($countergpsearch eq 1) | ($counterglsearch eq 1) | ($counterpksearch eq 1) | ($counterprsearch eq 1) | ($counterspsearch eq 1)| ($counterslsearch eq 1) | ($counterstsearch eq 1)|($countermainglycero eq 1)))
...
if ((($round eq 0)|($round > 0))&&($counterentirelipidome eq 0) && ($countermainhalo eq 0)&&  ($countermainglycero eq 0)&&($countercustomcheck eq 0) && ($countersulfo eq 0) && ($countermonoglycero eq 0)&&($counterdiglycero eq 0)&&($countertriglycero eq 0)&&($counterotherglycero eq 0)&&($countermainphospho eq 0) && ($countersphingophospho eq 0)&&($counterotherphospho eq 0)&& ($countermainphospho eq 0) && ($countersphingophospho eq 0) && ($countergpsearch eq 0) && ($counterotherphospho eq 0) && ($counterfasearch eq 0) && ($countergpsearch eq 0) && ($counterglsearch eq 0) && ($counterpksearch eq 0) && ($counterprsearch eq 0) && ($counterspsearch eq 0)&& ($counterslsearch eq 0) && ($counterstsearch eq 0)&&($counter2lihpos eq 0)&&($counterlipos eq 0)&&($counterli2hneg eq 0) && ($countermhpos eq 0)&& ($countermampos eq 0)&&($counternapos eq 0) && ($counternahpos eq 0)&& ($countermhneg eq 0)&&($counterachneg eq 0) && ($counternahneg eq 0)&&  ($countermmass eq 0) && ($counterclneg eq 0) &&($counterkpos eq 0)&& ($counter2khpos eq 0) &&($counterk2hneg eq 0))
{
$errortoptitle="Result of the query";
$errortitle="Oops ! There are no results matching to the query ";


if (($lvck > $uvck)|(($lvck eq "")&&($uvck eq "")))
{$errormess="Lower and Upper limit have \n not been entered properly.\nChoose Molecular Mass or a Type of Ion .\nSelect one or more lipid category(ies).\n";  }
else
{$errormess="\nChoose Molecular Mass or a Type of Ion .\nSelect one or more lipid category(ies).\n";  }
$errorbottom="Back";
...
$queriedpara=$nodata->Label(-text=>"\nQueried parameters -  Lower limit  : $lvck \t Upper limit : $uvck \t $qurdion \t  \n",-font=>"{times new roman} 12 bold",-background=>$col,-foreground=>$col5)->pack(-side=>'top',-fill=>x);
$goback=$nodata->Button (-text=>"$errorbottom",-background=>$col4,-foreground=>$col,-font=>"{$fontty} 18",-command=>\&entriesnodata)->pack(-side=>bottom,-fill=>x);
$label=$nodata->Label(-text=>"$errormess\n",-font=>"{times new roman} 36 ",-background=>$col,-foreground=>$col3)->pack(-side=>'bottom',-fill=>x);
...
if (($round eq 0)&&($counter2lihpos eq 0)&&($counterlipos eq 0)&&($counterli2hneg eq 0) && ($countermhpos eq 0)&& ($countermampos eq 0)&&($counternapos eq 0) && ($counternahpos eq 0)&& ($countermhneg eq 0)&&($counterachneg eq 0) && ($counternahneg eq 0)&&  ($countermmass eq 0) && ($counterclneg eq 0) &&($counterkpos eq 0)&& ($counter2khpos eq 0) &&($counterk2hneg eq 0)&&(($counterentirelipidome eq 1) | ($countermainhalo eq 1)| ($countercustomcheck eq 1) | ($countersulfo eq 1) | ($countermonoglycero eq 1) | ($counterdiglycero eq 1)|($countertriglycero eq 1)|($counterotherglycero eq 1)|($countermainphospho eq 1) | ($countersphingophospho eq 1)|($counterotherphospho eq 1)| ($countermainphospho eq 1) | ($countersphingophospho eq 1) | ($countergpsearch eq 1) | ($counterotherphospho eq 1) | ($counterfasearch eq 1) | ($countergpsearch eq 1) | ($counterglsearch eq 1) | ($counterpksearch eq 1) | ($counterprsearch eq 1) | ($counterspsearch eq 1)| ($counterslsearch eq 1) | ($counterstsearch eq 1)|($countermainglycero eq 1)))
{if ($lvck eq ""){$lvck="No value entered";}
if ($uvck eq ""){ $uvck="No value entered";}
if ($lvck>$uvck){$mess = "Enter Lower and Upper Limits Properly.";}
else{$mess="";}
$errortoptitle="Result of the query";
$errortitle="Oops ! There are no results matching to the query ";
$errormess="$mess\nChoose Molecular Mass or a Type of Ion .\n\n";  
...
if (($lvck<$uvck)&&($lvck != "")&&($uvck !="")&&($round >0)&&($countermainglycero eq 0)&&($counterentirelipidome eq 0) && ($countermainhalo eq 0)&&  ($countercustomcheck eq 0) && ($countersulfo eq 0) && ($countermonoglycero eq 0)&&($counterdiglycero eq 0)&&($countertriglycero eq 0)&&($counterotherglycero eq 0)&&($countermainphospho eq 0) && ($countersphingophospho eq 0)&&($counterotherphospho eq 0)&& ($countermainphospho eq 0) && ($countersphingophospho eq 0) && ($countergpsearch eq 0) && ($counterotherphospho eq 0) && ($counterfasearch eq 0) && ($countergpsearch eq 0) && ($counterglsearch eq 0) && ($counterpksearch eq 0) && ($counterprsearch eq 0) && ($counterspsearch eq 0)&& ($counterslsearch eq 0) && ($counterstsearch eq 0)&&((($counter2lihpos eq 1)|($counterlipos eq 1)|($counterli2hneg eq 1) | ($countermhpos eq 1) | ($countermampos eq 1)|($counternapos eq 1) | ($counternahpos eq 1) | ($countermhneg eq 1)|($counterachneg eq 1) | ($counternahneg eq 1)|  ($countermmass eq 1) | ($counterclneg eq 1) |($counterkpos eq 1)| ($counter2khpos eq 1) |($counterk2hneg eq 1))))
{
$errortoptitle="Result of the query";
$errortitle="Oops ! There are no results matching to the query";
$errormess="\n\nSelect one or more lipid category(ies).\n\n";  
...
if (($lvck>$uvck)&&($lvck != "")&&($uvck !="")&&($round  eq 0)&&($counterentirelipidome eq 0) &&($countermainglycero eq 0)&& ($countermainhalo eq 0)&&  ($countercustomcheck eq 0) && ($countersulfo eq 0) && ($countermonoglycero eq 0)&&($counterdiglycero eq 0)&&($countertriglycero eq 0)&&($counterotherglycero eq 0)&&($countermainphospho eq 0) && ($countersphingophospho eq 0)&&($counterotherphospho eq 0)&& ($countermainphospho eq 0) && ($countersphingophospho eq 0) && ($countergpsearch eq 0) && ($counterotherphospho eq 0) && ($counterfasearch eq 0) && ($countergpsearch eq 0) && ($counterglsearch eq 0) && ($counterpksearch eq 0) && ($counterprsearch eq 0) && ($counterspsearch eq 0)&& ($counterslsearch eq 0) && ($counterstsearch eq 0)&&(($counter2lihpos eq 1)|($counterlipos eq 1)|($counterli2hneg eq 1) | ($countermhpos eq 1) | ($countermampos eq 1)|($counternapos eq 1) | ($counternahpos eq 1) | ($countermhneg eq 1)|($counterachneg eq 1) | ($counternahneg eq 1)|  ($countermmass eq 1) | ($counterclneg eq 1) |($counterkpos eq 1)| ($counter2khpos eq 1) |($counterk2hneg eq 1)))
{
$errortoptitle="Result of the query";
$errortitle="Oops ! There are no results matching to the query ";
$errormess="\n\nSelect one or more lipid category(ies).\n\n";  
...
if (($lvck eq "" )&&($uvck eq "")&&(($round >0)|($round eq 0))&&($countermainglycero eq 0)&&($counterentirelipidome eq 0) && ($countermainhalo eq 0)&&  ($countercustomcheck eq 0) && ($countersulfo eq 0) && ($countermonoglycero eq 0)&&($counterdiglycero eq 0)&&($countertriglycero eq 0)&&($counterotherglycero eq 0)&&($countermainphospho eq 0) && ($countersphingophospho eq 0)&&($counterotherphospho eq 0)&& ($countermainphospho eq 0) && ($countersphingophospho eq 0) && ($countergpsearch eq 0) && ($counterotherphospho eq 0) && ($counterfasearch eq 0) && ($countergpsearch eq 0) && ($counterglsearch eq 0) && ($counterpksearch eq 0) && ($counterprsearch eq 0) && ($counterspsearch eq 0)&& ($counterslsearch eq 0) && ($counterstsearch eq 0)&&((($counter2lihpos eq 1)|($counterlipos eq 1)|($counterli2hneg eq 1) | ($countermhpos eq 1) | ($countermampos eq 1)|($counternapos eq 1) | ($counternahpos eq 1) | ($countermhneg eq 1)|($counterachneg eq 1) | ($counternahneg eq 1)|  ($countermmass eq 1) | ($counterclneg eq 1) |($counterkpos eq 1)| ($counter2khpos eq 1) |($counterk2hneg eq 1))))
{$lvck=$uvck="No value entered";
$errortoptitle="Result of the query";
$errortitle="Oops ! There are no results matching to the query";
$errormess="Enter Lower and Upper Limits Properly.\nSelect one or more lipid category(ies).\n\n";  
$errorbottom="Back";
$m->title("MS - LAMP - M. tb Lipidome MS-LAMP :$errortoptitle");
$nodata=$m->Frame(-background=>$col)->pack(-side=>top,-fill=>x);
$noquery=$nodata->Label(-text=>"\n$errortitle\n",-font=>"{times new roman} 24 bold",-background=>$col1,-foreground=>$col5,-borderwidth=>4,-relief=>ridge)->pack(-side=>'top',-fill=>x);
...
if (($lvck eq 0 )&&($uvck eq 0)&&(($round >0)|($round eq 0))&&($countermainglycero eq 0)&&($counterentirelipidome eq 0) && ($countermainhalo eq 0)&&  ($countercustomcheck eq 0) && ($countersulfo eq 0) && ($countermonoglycero eq 0)&&($counterdiglycero eq 0)&&($countertriglycero eq 0)&&($counterotherglycero eq 0)&&($countermainphospho eq 0) && ($countersphingophospho eq 0)&&($counterotherphospho eq 0)&& ($countermainphospho eq 0) && ($countersphingophospho eq 0) && ($countergpsearch eq 0) && ($counterotherphospho eq 0) && ($counterfasearch eq 0) && ($countergpsearch eq 0) && ($counterglsearch eq 0) && ($counterpksearch eq 0) && ($counterprsearch eq 0) && ($counterspsearch eq 0)&& ($counterslsearch eq 0) && ($counterstsearch eq 0)&&(($counter2lihpos eq 0)&&($counterlipos eq 0)&&($counterli2hneg eq 0) && ($countermhpos eq 0)&& ($countermampos eq 0)&&($counternapos eq 0) && ($counternahpos eq 0)&& ($countermhneg eq 0)&&($counterachneg eq 0) && ($counternahneg eq 0)&&  ($countermmass eq 0) && ($counterclneg eq 0) &&($counterkpos eq 0)&& ($counter2khpos eq 0)))
{$lvck=$uvck="No value entered";
$errortoptitle="Result of the query";
$errortitle="Oops ! There are no results matching to the query";
$errormess="Enter Lower and Upper Limits Properly.\nSelect one or more lipid category(ies).\n";  
...
if (($lvck eq "" )&&($uvck != "" )&&(($round >0)|($round eq 0))&&($countermainglycero eq 0)&&($counterentirelipidome eq 0) && ($countermainhalo eq 0)&&  ($countercustomcheck eq 0) && ($countersulfo eq 0) && ($countermonoglycero eq 0)&&($counterdiglycero eq 0)&&($countertriglycero eq 0)&&($counterotherglycero eq 0)&&($countermainphospho eq 0) && ($countersphingophospho eq 0)&&($counterotherphospho eq 0)&& ($countermainphospho eq 0) && ($countersphingophospho eq 0) && ($countergpsearch eq 0) && ($counterotherphospho eq 0) && ($counterfasearch eq 0) && ($countergpsearch eq 0) && ($counterglsearch eq 0) && ($counterpksearch eq 0) && ($counterprsearch eq 0) && ($counterspsearch eq 0)&& ($counterslsearch eq 0) && ($counterstsearch eq 0)&&((($counter2lihpos eq 1)|($counterlipos eq 1)|($counterli2hneg eq 1) | ($countermhpos eq 1) | ($countermampos eq 1)|($counternapos eq 1) | ($counternahpos eq 1) | ($countermhneg eq 1)|($counterachneg eq 1) | ($counternahneg eq 1)|  ($countermmass eq 1) | ($counterclneg eq 1) |($counterkpos eq 1)| ($counter2khpos eq 1) |($counterk2hneg eq 1))))
{$lvck="No value entered";
$errortoptitle="Result of the query";
$errortitle="Oops ! There are no results matching to the query ";
$errormess="Enter Lower and Upper Limits Properly.\nSelect one or more lipid category(ies).\n";  
$errorbottom="Back";
...
if (($uvck eq "" )&&($lvck != "" )&&(($round >0)|($round eq 0))&&($countermainglycero eq 0)&&($counterentirelipidome eq 0) && ($countermainhalo eq 0)&&  ($countercustomcheck eq 0) && ($countersulfo eq 0) && ($countermonoglycero eq 0)&&($counterdiglycero eq 0)&&($countertriglycero eq 0)&&($counterotherglycero eq 0)&&($countermainphospho eq 0) && ($countersphingophospho eq 0)&&($counterotherphospho eq 0)&& ($countermainphospho eq 0) && ($countersphingophospho eq 0) && ($countergpsearch eq 0) && ($counterotherphospho eq 0) && ($counterfasearch eq 0) && ($countergpsearch eq 0) && ($counterglsearch eq 0) && ($counterpksearch eq 0) && ($counterprsearch eq 0) && ($counterspsearch eq 0)&& ($counterslsearch eq 0) && ($counterstsearch eq 0)&&((($counter2lihpos eq 1)|($counterlipos eq 1)|($counterli2hneg eq 1) | ($countermhpos eq 1) | ($countermampos eq 1)|($counternapos eq 1) | ($counternahpos eq 1) | ($countermhneg eq 1)|($counterachneg eq 1) | ($counternahneg eq 1)|  ($countermmass eq 1) | ($counterclneg eq 1) |($counterkpos eq 1)| ($counter2khpos eq 1) |($counterk2hneg eq 1))))
{$uvck="No value entered";
$errortoptitle="Result of the query";
$errortitle="Oops ! There are no results matching to the query";
$errormess="Enter Lower and Upper Limits Properly.\nSelect one or more lipid category(ies).\n";  
$errorbottom="Back";
...
if (($round eq 0)&&(($uvck eq 0 )|($uvck > 0 ))&&(($lvck eq 0 )|($lvck > 0 ))&&($lvck < $uvck)&&(($counter2lihpos eq 1)|($counterlipos eq 1)|($counterli2hneg eq 1) | ($countermhpos eq 1)| ($countermampos eq 1)|($counternapos eq 1)| ($counternahpos eq 1)| ($countermhneg eq 1)|($counterachneg eq 1) | ($counternahneg eq 1)|  ($countermmass eq 1) | ($counterclneg eq 1) |($counterkpos eq 1)| ($counter2khpos eq 1) )&&($countermainglycero eq 0)&&($counterentirelipidome eq 0) && ($countermainhalo eq 0)&&  ($countercustomcheck eq 0) && ($countersulfo eq 0) && ($countermonoglycero eq 0)&&($counterdiglycero eq 0)&&($countertriglycero eq 0)&&($counterotherglycero eq 0)&&($countermainphospho eq 0) && ($countersphingophospho eq 0)&&($counterotherphospho eq 0)&& ($countermainphospho eq 0) && ($countersphingophospho eq 0) && ($countergpsearch eq 0) && ($counterotherphospho eq 0) && ($counterfasearch eq 0) && ($countergpsearch eq 0) && ($counterglsearch eq 0) && ($counterpksearch eq 0) && ($counterprsearch eq 0) && ($counterspsearch eq 0)&& ($counterslsearch eq 0) && ($counterstsearch eq 0))
{
$errortoptitle="Result of the query";
$errortitle="Oops ! There are no results matching to the query ";
$errormess="\nSelect one or more lipid category(ies).\n\n";  
...
$headcom="";
if ($masschoice eq 0){$headcom="(Monoisotopic)";}
if ($masschoice eq 1){$headcom="(Average)";}


$m->title("MS - LAMP - M. tb Lipidome MS-LAMP -m/z range : Result of the query for queried parameters -  Lower m/z limit  : $lowerfp $headcom  ,\t Upper m/z limit  : $upperfp $headcom\t Ion type : $qurdion ");
&entriessearching;
...
$fa=0;$gp=0;$gl=0;$pk=0;$pr=0;$sl=0;$sp=0;$st=0;$cq=0;$sfl=0;$flor=0;$clor=0;$brom=0;$iod=0;$mak=0;$mac=0;$diac=0;$diak=0;$diacak=0;$diakac=0;$triac=0;$all=0;$spl=0;$opl=0;
@lstrfa=0;@lstrgl=0;@lstrgp=0;@lstrpk=0;@lstrpr=0;@lstrst=0;@lstrsl=0;@lstrsp=0;$ogll=0;
...
$ressps=$structureslabel->Label( -text=>"" ,-font=>"{Comic Sans Ms} 2 ",-background =>$col1 ,-foreground => $col5)->pack(-side=>top,-fill=>x,-anchor=>center);
$resvaria=$structureslabel->Label( -text=>"Queried Parameters -   Lower m/z limit  : $lowerfp $headcom  \t Upper m/z limit  : $upperfp $headcom\t Ion type : $qurdion  ",-background =>white ,-foreground => black,-font=>"{Times new roman} 11 ")->pack(-side=>top,-fill=>x,-anchor=>center);		
$ressps=$structureslabel->Label( -text=>"" ,-font=>"{Comic Sans Ms} 4 bold",-background =>$col1 ,-foreground => $col5)->pack(-side=>top,-fill=>x,-anchor=>center);
...
$csv=$checkfor1->Checkbutton(-text => ".csv  ",-command => \&csvoutput,-background=>"light blue",-foreground=>black,-font=>"{calibri} 13")->pack(-side => 'left');
$txt=$checkfor1->Checkbutton(-text => ".txt ",-command => \&txtoutput,-background=>"light yellow",-foreground=>black,-font=>"{calibri} 13")->pack(-side => 'left');
#$xls=$checkfor1->Checkbutton(-text => ".xls ",-command => \&xlsoutput,-background=>"light pink",-foreground=>black,-font=>"{calibri} 13")->pack(-side => 'left');
...
{	print OUT "\n\n<<<< FATTY ACYLS >>>>\n\n";
	&csvouthead;
for	(sort keys %data)
	{if (($ll<$ul)&&($_>$ll) && ($_<$ul) && ( $data{$_} =~ /MMFA/))
		{&csvoutmain;}
	}
}
...
{	print OUT "\n\n<<<< GLYCEROLIPIDS >>>>\n\n";
	&csvouthead;
for(sort keys %data)
	{if (($ll<$ul)&&($_>$ll) && ($_<$ul) && ( $data{$_} =~ /MMGL/))
	{&csvoutmain;}
	}
};
...
{print OUT "\n\n<<<< GLYCEROPHOSPHOLIPIDS >>>>\n\n";
	&csvouthead;
for	(sort keys %data)
	{if (($ll<$ul)&&($_>$ll) && ($_<$ul)&& ( $data{$_} =~ /MMGP/))
	{&csvoutmain;}
	}
}
...
&csvouthead;
for(sort keys %data)
	{if (($ll<$ul)&&($_>$ll) && ($_<$ul) && ( $data{$_} =~ /MMPK/))
	{&csvoutmain;}
	}
}
...
&csvouthead;
for(sort keys %data)
	{if (($ll<$ul)&&($_>$ll) && ($_<$ul)&& ( $data{$_} =~ /MMPR/))
	{&csvoutmain;}
	}
}
...
&csvouthead;
for(sort keys %data)
	{if (($ll<$ul)&&($_>$ll) && ($_<$ul) && ( $data{$_} =~ /MMSL/))
	{&csvoutmain;}
	}
}
...
&csvouthead;
for(sort keys %data)
	{if (($ll<$ul)&&($_>$ll) && ($_<$ul) && ( $data{$_} =~ /MMSP/))
	{&csvoutmain;}
	}
}
...
&csvouthead;
for(sort keys %data)
	{if (($ll<$ul)&&($_>$ll) && ($_<$ul) && ( $data{$_} =~ /MMST/))
{&csvoutmain;		}
	}
}
...
&csvouthead;
for(sort keys %data)
{if (($ll<$ul)&&($_>$ll) && ($_<$ul) && ( $m2sc{$_} =~ /$customquery/))
{&csvoutmain;}}
}
...
&csvouthead;
for(sort keys %data)
{if (($ll<$ul)&&($_>$ll) && ($_<$ul) && ( ($m2f{$_} =~ /F/) |($m2f{$_} =~ /Cl/) |($m2f{$_} =~ /Br/) | ($m2f{$_} =~ /I/) ))
{&csvoutmain;}}}
...
&csvouthead;
for(sort keys %data)
{if (($ll<$ul)&&($_>$ll) && ($_<$ul) && (($m2mc{$_} =~ /GL01/)|($m2mc{$_} =~ /GL02/) |($m2mc{$_} =~ /GL03/)|($m2mc{$_} =~ /GL00/)|($m2mc{$_} =~ /GL04/) |($m2mc{$_} =~ /GL05/)))
{&csvoutmain;}}}
...
&csvouthead;
for(sort keys %data)
{if (($ll<$ul)&&($_>$ll) && ($_<$ul) && ($m2mc{$_} =~ /GL01/))
{&csvoutmain;}}}
...
&csvouthead;
for(sort keys %data)
{if (($ll<$ul)&&($_>$ll) && ($_<$ul) && ($m2sc{$_} =~ /GL0101/))
{&csvoutmain;}}}
...
&csvouthead;
for(sort keys %data)
{if (($ll<$ul)&&($_>$ll) && ($_<$ul) && ($m2sc{$_} =~ /GL0102/))
{&csvoutmain;}}}
...
&csvouthead;
for(sort keys %data)
{if (($ll<$ul)&&($_>$ll) && ($_<$ul) && ($m2sc{$_} =~ /GL02/))
{&csvoutmain;}}}
...
&csvouthead;
for(sort keys %data)
{if (($ll<$ul)&&($_>$ll) && ($_<$ul) && ($m2sc{$_} =~ /GL0203/))
{&csvoutmain;}}}
...
&csvouthead;
for(sort keys %data)
{if (($ll<$ul)&&($_>$ll) && ($_<$ul) && ($m2sc{$_} =~ /GL0201/))
{&csvoutmain;}}}
...
&csvouthead;
for(sort keys %data)
{if (($ll<$ul)&&($_>$ll) && ($_<$ul) && ($m2sc{$_} =~ /GL0207/))
{&csvoutmain;}}}
...
&csvouthead;
for(sort keys %data)
{if (($ll<$ul)&&($_>$ll) && ($_<$ul) && ($m2sc{$_} =~ /GL0202/))
{&csvoutmain;}}}
...
&csvouthead;
for(sort keys %data)
{if (($ll<$ul)&&($_>$ll) && ($_<$ul) && ($m2mc{$_} =~ /GL03/))
{&csvoutmain;}}}
...
&csvouthead;
for(sort keys %data)
{if (($ll<$ul)&&($_>$ll) && ($_<$ul) && (($m2mc{$_} =~ /GL00/)|($m2mc{$_} =~ /GL04/)|($m2mc{$_} =~ /GL05/)))
{&csvoutmain;}}}
...
&csvouthead;
for(sort keys %data)
{if (($ll<$ul)&&($_>$ll) && ($_<$ul) && (($m2f{$_} =~ /P/) && ($data{$_} =~ /MMGP/)) )
{&csvoutmain;}}}
...
&csvouthead;
for(sort keys %data)
{if (($ll<$ul)&&($_>$ll) && ($_<$ul) && (($m2f{$_} =~ /P/) && ($data{$_} =~ /MMSP/)) )
{&csvoutmain;}}}
...
&csvouthead;
for(sort keys %data)
{if (($ll<$ul)&&($_>$ll) && ($_<$ul) && (($m2f{$_} =~ /P/ ) && (( $data{$_} =~ /MMFA/) | ( $data{$_} =~ /MMGL/) | ( $data{$_} =~ /MMSL/) | ( $data{$_} =~ /MMST/) | ( $data{$_} =~ /MMPR/) | ( $data{$_} =~ /MMPK/))) )
{&csvoutmain;}}}
...
sub csvoutmain{	$code = substr($_,0,-2);
		$formforcsv=substr($m2f{$_},1,-4);
		chomp $m2sn{$_};
		chomp $m2c{$_};
		chomp $m2mc{$_};
		print OUT "\"$code\",\"$formforcsv\",$m2sn{$_},$m2c{$_},$m2mc{$_},$m2sc{$_}";
		}
...
if ($masschoice eq 0){$masschoiceforprinttxt ="M-Monoisotopic";}
if ($masschoice eq 1){$masschoiceforprinttxt ="M-Average";}
...
sub textouthead{print OUT "$masschoiceforprinttxt (db)*\tMolecular Formulae\t\t\tSubclass\t\t\t\t\tScientific Name\n\n";};
...
sub textoutmain{$code = substr($_,0,-2);
		...
		$m2monotxt=substr($m2mono{$_},1,-2);
		print OUT "$masse\t\t$m2ftxt\t\t$m2sctxt\t\t\t$m2sntxt\n";
		};
...
$formatheading=$workbook->add_format(); 
	...
	$summary->write($rowsummary,$columnsummary,'Lower m/z Limit',$formatname);$columnsummary++;
	$summary->write($rowsummary,$columnsummary,$lower,$format);$columnsummary++;
	$summary->write($rowsummary,$columnsummary,'Upper m/z Limit',$formatname);$columnsummary++;
	$summary->write($rowsummary,$columnsummary,$upper,$format);$columnsummary++;
	$summary->write($rowsummary,$columnsummary,'Queried Ion',$formatname);$columnsummary++;
	$summary->write($rowsummary,$columnsummary,$qurdion,$format);$columnsummary++;
...
if ($masschoice eq 1){$all->write($row, $col, 'M-Average (db)*', $format);$col++;}
	...
	$all->write($row, $col,'[M-H]- (db)',$format);$col++;	
	$all->write($row, $col,'[M+Cl]- (db)',$format);$col++;
	$all->write($row, $col,'[M+HOAc-H]- (db)',$format);$col++;
	$all->write($row, $col,'[M+Li-2H]- (db)',$format);$col++;
	$all->write($row, $col,'[M+Na-2H]- (db)',$format);$col++;
	$all->write($row, $col,'[M+K-2H]- (db)',$format);$col++;

	$all->write($row, $col,'[M+H]+ (db)',$format);$col++;		
	$all->write($row, $col,'[M+NH4]+ (db)',$format);$col++;
	$all->write($row, $col,'[M+Li]+ (db)',$format);$col++;
	$all->write($row, $col,'[M+Na]+ (db)',$format);$col++;	
	$all->write($row, $col,'[M+K]+ (db)',$format);$col++;
	$all->write($row, $col,'[M+2Li-H]+ (db)',$format);$col++;
	$all->write($row, $col,'[M+2Na-H]+ (db)',$format);$col++;
	$all->write($row, $col,'[M+2K-H]+ (db)',$format);$col++;
	$row=1;
}
sub res
{$code = substr($_,0,-2);
...
$m2monofp=substr($m2mono{$_},1,-2);

		$all->write($row,$column,$masse);$column++;
...
if ($masschoice eq 0)
{
$ionmhneg=($code-$ionmhnegvalmono);$all->write($row, $column, $ionmhneg);$column++;	
$ionclneg=($code-$ionclnegvalmono);$all->write($row, $column, $ionclneg);$column++;
$ionachneg=($code-$ionachnegvalmono);$all->write($row, $column, $ionachneg);$column++;
$ionli2hpos=($code-$ionlihnegvalmono);$all->write($row, $column, $ionli2hpos);$column++;
$ionnahneg=($code-$ionnahnegvalmono);$all->write($row, $column, $ionnahneg);$column++;
$ionk2hpos=($code-$ionk2hnegvalmono);$all->write($row, $column, $ionk2hpos);$column++;

$ionmhpos=($code-$ionmhposvalmono);$all->write($row, $column, $ionmhpos);$column++;	
$ionmampos=($code-$ionmamposvalmono);$all->write($row, $column, $ionmampos);$column++;
$ionlipos=($code-$ionliposvalmono);$all->write($row, $column, $ionlipos);$column++;	
$ionnapos=($code-$ionnaposvalmono);$all->write($row, $column, $ionnapos);$column++;	
$ionkpos=($code-$ionkposvalmono);$all->write($row, $column, $ionkpos);$column++;	
$ion2lihpos=($code-$ion2lihposvalmono);$all->write($row, $column, $ion2lihpos);$column++;
$ionnahneg=($code-$ion2nahposvalmono);$all->write($row, $column, $ionnahneg);$column++;
$ion2khpos=($code-$ion2khposvalmono);$all->write($row, $column, $ion2khpos);$column++;		
$row++;

}
elsif ($masschoice eq 1)
{$ionmhneg=($code-$ionmhnegvalavg);$all->write($row, $column, $ionmhneg);$column++;	
$ionclneg=($code-$ionclnegvalavg);$all->write($row, $column, $ionclneg);$column++;
$ionachneg=($code-$ionachnegvalavg);$all->write($row, $column, $ionachneg);$column++;
$ionli2hpos=($code-$ionlihnegvalavg);$all->write($row, $column, $ionli2hpos);$column++;
$ionnahneg=($code-$ionnahnegvalavg);$all->write($row, $column, $ionnahneg);$column++;
$ionk2hpos=($code-$ionk2hnegvalavg);$all->write($row, $column, $ionk2hpos);$column++;

$ionmhpos=($code-$ionmhposvalavg);$all->write($row, $column, $ionmhpos);$column++;	
$ionmampos=($code-$ionmamposvalavg);$all->write($row, $column, $ionmampos);$column++;
$ionlipos=($code-$ionliposvalavg);$all->write($row, $column, $ionlipos);$column++;	
$ionnapos=($code-$ionnaposvalavg);$all->write($row, $column, $ionnapos);$column++;	
$ionkpos=($code-$ionkposvalavg);$all->write($row, $column, $ionkpos);$column++;	
$ion2lihpos=($code-$ion2lihposvalavg);$all->write($row, $column, $ion2lihpos);$column++;
$ionnahneg=($code-$ion2nahposvalavg);$all->write($row, $column, $ionnahneg);$column++;
$ion2khpos=($code-$ion2khposvalavg);$all->write($row, $column, $ion2khpos);$column++;		
